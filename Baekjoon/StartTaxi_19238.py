...
def input_func():
    ...
    customers.sort(key = lambda customer : [customer.current_x, customer.current_y])
    return road_map, taxi, customers

def calc_distance(road_map, cur_x, cur_y, dest_x, dest_y):
    queue = deque()
...
def pick_customer(road_map,taxi_x, taxi_y, customers):
    ...
    for idx, customer in enumerate(customers):
        distance = calc_distance(road_map, taxi_x, taxi_y, customer.current_x, customer.current_y)
...
def run(road_map, taxi, customers):
    while True:
        ...
        # 손님을 픽업
        taxi.move(picked_customer.current_x, picked_customer.current_y, move_distance)
        # print(f"after move taxi's location : [{taxi.x},{taxi.y}]")
        if taxi.remain_fuel <= 0:
            return -1

        # 손님을 목적지까지 이동
        to_destination_distance = calc_distance(road_map, taxi.x, taxi.y, picked_customer.dest_x, picked_customer.dest_y)
